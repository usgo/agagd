...
class Country(models.Model):
    country_code = models.CharField(
        max_length=2, primary_key=True, db_column="Country_Code"
    )
...
class Tournament(models.Model):
    tournament_code = models.CharField(
        max_length=20, primary_key=True, db_column="Tournament_Code"
    )
    ...
    def __str__(self):
        return "{0} - on {1} with {2} players".format(
            self.tournament_code, self.tournament_date, self.total_players
        )
...
class Game(models.Model):
    ...
    def __str__(self):
        return "Tournament {0} Round {1}, {2} vs {3}".format(
            self.tournament_code, self.round, self.pin_player_1, self.pin_player_2
        )
