...
def tournament_list_helper(player_id, game_list=None):
    """ Creates a list with the tournament data from the player's games. """
    tournament_data = {}

    for game in game_list:
        temp_tournament_data = tournament_data.get(game.tournament_code.pk, {})
        ...
        temp_tournament_data["date"] = temp_tournament_data("date", game_date)
        ...
        tournament_data[game.tournament_code.pk] = temp_tournament_data
...
def index(request):
    latest_games = agagd_models.Game.objects.all().order_by("-game_date")[:25]
